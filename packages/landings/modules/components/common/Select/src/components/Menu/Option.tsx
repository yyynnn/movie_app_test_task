...
const Option = memo<OptionProps>(
  ({
    index,
    style,
    data: { menuOptions, selectOption, renderOptionLabel, focusedOptionIndex }
  }) => {
    const opt = menuOptions[index]

    const className = buildOptionClass(opt.isDisabled, opt.isSelected, index === focusedOptionIndex)

    return (
      <div style={style} className={className} onClick={() => selectOption(opt)}>
