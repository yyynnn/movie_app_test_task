...
import { ARIA_LIVE_CONTEXT_ID, ARIA_LIVE_SELECTION_ID } from '../../constants'
import type { AriaLiveAttribute, FocusedOption, SelectedOption } from '../../types'
...
const AriaLiveRegion: React.FC<AriaLiveRegionProps> = ({
  ...
  const menuMsg = menuOpen
    ? `Use Up and Down to choose options${
        isDisabled ? '' : ', press Enter or Tab to select the currently focused option'
      }, press Escape to close the menu.`
    : `${ariaLabel} is focused${
        isSearchable ? ', type to filter options' : ''
      }, press Down arrow key to open the menu.`

  const focusedMsg = label
    ? `Option ${label} is ${isSelected ? 'selected' : 'focused'}${isDisabled ? ' disabled' : ''}, ${
        index + 1
      } of ${optionCount}.`
    : ''

  const optionsMsg = `${optionCount} option(s) available${
    inputValue ? ' for search ' + inputValue : ''
  }.`
  ...
  return (
    <A11yText aria-atomic="false" aria-live={ariaLive} aria-relevant="additions text">
