...
import styled, { type DefaultTheme, css, ThemeProvider } from 'styled-components'

import { AriaLiveRegion, AutosizeInput, IndicatorIcons, Menu, Value } from './components'
import {
...
const Select = forwardRef<SelectRef, SelectProps>(
  (
    ...
    const theme = useMemo<DefaultTheme>(() => {
      return isPlainObject(themeConfig) ? mergeDeep(DEFAULT_THEME, themeConfig) : DEFAULT_THEME
    }, [themeConfig])
    ...
    // Custom hook abstraction that handles
    // calculating menuHeightCalc (defaults to menuMaxHeight)
    // handles executing callbacks/logic on menuOpen state change.
    ...
    const openMenuAndFocusOption = useCallback(
      (position: OptionIndexEnum): void => {
        ...
        const selectedIdx = !isMulti ? menuOptions.findIndex((x) => x.isSelected) : -1
    ...
    useImperativeHandle(
      ...
      () => ({
        ...
        setValue: (option?: OptionData) => {
          setSelectedOption(normalizeValue(option, getOptionValueFn, getOptionLabelFn))
        },
    ...
    useUpdateEffect(() => {
      ...
      const inputChanged =
        curLength > 0 && (async || curLength !== options.length || prevLength === 0)
      const menuOpenAndOptionsGrew =
        menuOpenRef.current && prevLength !== undefined && prevLength < curLength
    ...
    const focusValueOnArrowKey = (key: string): void => {
      ...
      const curFocusedIdx = focusedMultiValue
        ? selectedOption.findIndex((x) => x.value === focusedMultiValue)
        : -1
      ...
      if (key === 'ArrowRight') {
        ...
        focusedIdx =
          curFocusedIdx !== 0 ? (curFocusedIdx === -1 ? lastValueIdx : curFocusedIdx - 1) : 0
      }

      const nextFocusedVal = focusedIdx > -1 ? selectedOption[focusedIdx].value! : null
        ...
        case 'Backspace': {
          ...
          if (focusedMultiValue) {
            ...
            const nextFocusedMultiValue =
              focusedIdx > -1 && focusedIdx < selectedOption.length - 1
