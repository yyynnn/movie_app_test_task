...
const Nearmiss = ({ selectedTicketData }) => {
  ...
  function handleCloseModal() {
    const elementModal = document.getElementById(selectedTicketData ? 'ticketModal' : 'nearmiss')
                ...
                <div className="wrap-info">
                  ...
                  <div className="select-wrapper">
                    <div className="select">
                      ...
                      >
                        {!isLoadingEmployee &&
                          employee.map((e) => (
                            <option key={e.id}>{`${e.name} ${e?.surname}`}</option>
                          ))}
                      </select>
                    </div>
                  </div>
                  <div className="select-wrapper">
                    <div className="select workcenter">
                      ...
                      >
                        {!isLoadingWorkCenter &&
                          workCenter.map((wc) => <option key={wc.id}>{wc.number}</option>)}
                      </select>
                    </div>
                  </div>
                      ...
                      >
                        {!isLoadingEmployee &&
                          selectedTicketData.rootCauses.map((e) => (
                            <option key={e.id}>{`${e.root_cause_category}`}</option>
                          ))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="wrap-file">
                    <label className="file flex">
                      <input
                        ...
                        onChange={(event) => {
                          convertFile(event)
                          setFieldValue('selectedFile', event.target.files[0].name)
                        }}
                      />
                      {selectedFile && (
                        <img
                      ...
                      {!selectedFile && errors.selectedFile && touched.selectedFile && (
                        <p className="error mt-1 mb-0 errorFile">{errors.selectedFile}</p>
                      )}
                    </label>
                  ...
                  <div className="wrap-file wrap-edit">
                    <div className="mt-4">
                      <label className="d-flex file-edit">
                        <input name="selectedFile" className="cursor-pointer " type="file" />
                        {values.selectedFile && (
                          <img
                            className="w-100 h-100 borderRounded"
                            // src={
                      ...
                      <div className="d-flex flex-column">
                        ...
                        {correctiveActions.map((action, index) => (
                          <CorrectiveAction
                            ...
                            handleChangeInput={(event) => handleChangeInput(index, event)}
                            handlePlus={handlePlus}
                            handleMinus={(event) => handleMinus(index, event)}
                          />
                        ))}
                      </div>
                    </div>
