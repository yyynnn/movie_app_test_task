...
export const AllTicketsPage = () => {
  ...
  const { data: ticketsData, isLoading, ...rest } = useGetPaginatedTicketList({ 'page[size]': pageSize, 'page[number]': page })
  const { data: tickets }: any = ticketsData || {}

  return <TableConstructor {...rest} data={tickets} isLoading={isLoading} page={page} pageSize={pageSize} setPageSize={setPageSize} setPage={setPage} rowId="ticket_id" />
}
